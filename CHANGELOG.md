...
- 回退多阶段 Docker 构建（实测反而更慢），恢复为“宿主机并行交叉编译 + Docker COPY 预编译产物”；保留 registry 缓存、每日清理等 CI 优化

## [v2.5.49] - 2026-08-30

### ⚙️ 验证

- 复用 v2.5.48 建立的 registry 缓存（Go 依赖层），验证跨 tag 命中后的构建提速；无功能改动

## [v2.5.48] - 2026-08-30

### ⚙️ 构建

- Docker 改为多阶段构建：Go 依赖层单独 `go mod download` 并走 registry 缓存（跨 tag 复用），运行阶段仅 COPY 预编译产物；版本号改为构建参数传入；宿主机仅保留 `go vet`/`go test`

## [v2.5.47] - 2026-08-30

### ⚙️ 验证

- 复用上一版建立的 Go 缓存（actions/cache），验证缓存命中后的构建提速；无功能改动
...
- 主题切换改为「颜色级」平滑过渡：只在切换瞬间对背景/文字/边框/描边应用 transition，让每组颜色渐变，不再是整页画面做图片式交叉淡化

## [v2.5.43] - 2026-08-30

### ✨ 体验优化

- 软导航淡入淡出放缓（淡出 240ms / 淡入 520ms）并减小位移（淡出上移 4px、淡入上移 12px），过渡更柔和
- 主题切换（亮/暗/跟随系统）启用 View Transition 平滑交叉淡入，不再是生硬跳色

## [v2.5.42] - 2026-08-30

### 🐛 修复

- 软导航切换后内容空白：淡出动画 `fill:forwards` 结束后会把主内容保持透明，现在淡入动画补 `fill:both` 并在每次导航前清除旧填充动画，确保切换后内容可见

## [v2.5.41] - 2026-08-30

### ✨ 体验优化

- 顶部菜单与站内链接改为软导航（fetch + 替换主内容）：旧页淡出、新页自下而上淡入，不再整页刷新/重排，消除点击菜单时的抖动
- 软导航同步更新页面标题、导航当前项高亮，并正确初始化各页脚本（如优选生成器）；支持浏览器前进/后退

## [v2.5.40] - 2026-08-30

### 🐛 修复

- 前端静态资源缓存免疫：站点缓存中间件跳过 `/static/`，页面引用的 CSS/JS 地址带版本号，避免发布新版本后浏览器仍加载旧的 `index.js`/`index.css` 导致链接行为不变

## [v2.5.39] - 2026-08-30

### 🐛 修复

- 首页/客户端页链接在前置 nginx `/show/ → /` 时丢失 `/show`：链接改为按浏览器地址推断部署前缀（服务端注入非空前缀优先，否则从 location 推断），订阅链接与一键导入深链统一由前端 JS 拼接
- 客户端页「本地部署时配置文件 / 筛选节点示例」改为前端 JS 拼接，与其余链接一致，避免服务端拿不到前缀导致不一致

## [v2.5.38] - 2026-08-30

### 🐛 修复

- CDN 判断健壮性：官方 IP 段更新失败时保留上一次成功的数据（不再清空 ranges）；ip-api 在线查询加 30 分钟结果缓存 + 去重，减少外部请求与结果抖动
- 关键词匹配优化：通用词（CDN / EDGE / ANYCAST / CACHE）改为按词边界匹配，避免 `EdgeHosting` 之类的非 CDN 名称误判；品牌词仍子串匹配；逻辑统一收敛到 `pkg/cdn.MatchOrg`，消除 geoip 与 cdn 两份重复列表

## [v2.5.37] - 2026-08-29

### 🎨 体验优化

- 首页统计卡（抓取源 / 全部节点 / 可用节点 / 优选 IP 节点）图标与标题统一上对齐：此前各卡内容高度不同，图标采用垂直居中对齐导致高低不一

## [v2.5.36] - 2026-08-29

### 🐛 修复

- 服务端拼接的链接（本地部署时配置文件 / 筛选节点示例）缺失部署前缀：将路由层依据请求路径/反代头推断出的前缀回传给模板，与前端 `window.PROXYPOOL_BASE_PATH` 保持一致
- 本地根路径访问保持无前缀；nginx `/show/ → /`（保留路径或反代头部告知）场景下，前后端链接都带 `/show`

## [v2.5.35] - 2026-08-29

### 🎨 体验优化

- 顶部菜单整页跳转启用浏览器原生 View Transition：旧页淡出、新页上移淡入，替代硬刷新造成的闪白/抖动（Chrome/Edge 生效，其余浏览器自动回退为普通跳转，不影响功能）

## [v2.5.34] - 2026-08-29

### 🎨 体验优化

- 优选页下拉框改用自定义箭头并留出右间距，不再贴边
- 根背景跟随明暗主题，导航/页面切换不再闪现白底

### 🐛 修复

- 部署前缀：应用不再从 URL 主动猜 `/show`，改为按 `config.base_path` / `X-Forwarded-Prefix` / `X-Forwarded-URI` 取前缀；本地部署保持根路径，nginx `/show/ → /` 场景由反代头部告知后自动带 `/show`
- 业务链接主机名按实际请求来源（`X-Forwarded-Host`/`X-Forwarded-Proto`，否则请求 Host/协议），不再使用 `config.domain`
- 修复客户端页「本地部署时配置文件 / 筛选节点示例」等链接被全局状态锁死：首个请求的域名/前缀会污染后续请求，现改为每次请求用副本渲染
- 站点缓存 key 并入实际来源与部署前缀，避免不同主机/端口/前缀命中同一缓存而串内容

## [v2.5.33] - 2026-08-29

### 🎨 布局微调

- 首页「运行状态」卡片：标题与测速/优选更新/抓取更新/GeoIP 状态值改为一屏单行显示；空间不足时状态项整体换行，避免文字内部折断

## [v2.5.32] - 2026-08-29

### 🎨 页面布局与配色改版（基于 v2.5.30）

- 回退 v2.5.31 首页重写，在 v2.5.30 基础上重做布局与配色：靛蓝→紫罗兰渐变主色 + 语义色（绿/琥珀/天蓝），暗色模式同步优化
- 首页新增页头（在线状态徽标 + 标题 + 更新时间）、图标统计卡（抓取源/全部节点/可用节点/优选 IP）、运行状态条
- 客户端改为图标卡片；订阅列表显示完整订阅地址并新增搜索过滤；任务按钮按功能分色
- 导航毛玻璃吸顶 + 渐变 Logo；客户端页/优选 IP 页统一卡片风格；新增内联 SVG favicon

### 🐛 修复

- 主题切换按钮无效：暗色变体由跟随系统改为 `.dark` 类控制（Tailwind `@custom-variant`），亮/暗/跟随系统三态切换、localStorage 持久化与防闪恢复正常

## [v2.5.31] - 2026-08-29

### 🎨 首页重写为 shadcn 风格（无 CDN）

- 白卡 + 浅灰页面 + 浅边框层级（对齐参考设计）
- 主按钮改黑色（bg-slate-900），次要按钮 Outline，状态色绿/黄/蓝
- 新增 Hero 区（在线徽章 + 大标题 + 描述 + 更新时间）、Notice 卡片
- Overview 统计卡（图标块 + 大数字）+ 运行状态卡（含刷新按钮）
- 客户端卡片 SaaS 化（图标/名称/描述/双按钮/底部链接）
- 订阅表升级 Data Table：协议徽章 + 名称 + URL + 复制（带"已复制"状态）+ 搜索过滤
- 图标全部内联 SVG（无 Lucide CDN），复制/主题/best_token 逻辑保留

## [v2.5.30] - 2026-08-29

### 🎨 导航微调

- 导航底色纯白 → 浅灰 slate-100（比页面主体略深一档，层次更柔和）

## [v2.5.29] - 2026-08-29

### 🎨 导航改浅色方案（CodexHost 风格）

- 导航：深灰 → 白底浅色（bg-white + 深色文字 + 底部细边框），深色模式自动反转为深底
- 当前页高亮：浅蓝底（bg-blue-50）+ 深色文字（对齐参考风格）
- hover：浅灰底（bg-slate-100），深色模式 hover:bg-white/10

## [v2.5.28] - 2026-08-29

### 🎨 导航与强调色去 AI 化

- 导航：紫色渐变（indigo→violet）→ 纯深灰 slate-900（深浅模式统一深色导航，深色模式下与页面分层）
- 强调色：indigo 紫 → blue 蓝（按钮/链接/焦点环），整体更工具感

## [v2.5.27] - 2026-08-29

### 🐛 修复

- 导航汉堡断点 lg(1024) → md(768)：电脑/平板（≥768px）一律横向菜单不出现汉堡；仅手机与 iPad mini 竖屏（<768px）用汉堡
- 菜单项 padding 微调（px-2.5）保证 768px 宽度容纳全部 7 项

## [v2.5.26] - 2026-08-29

### 🐛 修复

- Loon 图标改为热气球（内联 SVG，metron 字体无 loon 字形），导航与页面标题统一
...
- 按钮从品牌旁移到导航菜单内（桌面菜单右侧 / 手机菜单底部带分隔线）
- 图标从 emoji（🌙/☀️/🔄）换成内联 SVG（月亮/太阳/半日半月），更清晰统一

## [v2.5.23] - 2026-08-29

### 🐛 修复

- 导航汉堡菜单断点 sm → lg：iPad 竖屏（744/768/834px）菜单项被挤出屏幕，<1024px 一律用汉堡菜单
...
- clash 页「本地部署时配置文件」地址改为实际站点域名（原 127.0.0.1 在部署站点上显示错误）
...
- 新增 sort 排序参数（country/ip/random，默认固定排序国家→IP→端口），仅 bestProxyIp/bestIpKr 支持
...
- 修正静态表格筛选参数文档（country → ?d= 落地国家 + ?c= 节点国家代码）
- 保留下方静态订阅端点表格
